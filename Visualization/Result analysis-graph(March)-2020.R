...
library(reshape2)
library(ggplot2)
data <- read_excel("N:/DaheeKim/March/Result_analysis-graphs-2020(March).xlsx", sheet = "plot1")
head(data)
...
#(2)
ls <- c("Total phytoplankton","Maturity index")
melt1 <- melt[which(!melt$variable %in% ls),]
melt2 <- melt[which(melt$variable %in% ls),]
melt1$Type <- rep(c("Higher life forms","Algal family"),c(24,24))
melt2$Type <- rep(c("Higher life forms","Algal family"),c(6,6))
melt1$Type <- factor(melt1$Type, levels=c("Higher life forms","Algal family"))
melt2$Type <- factor(melt2$Type, levels=c("Higher life forms","Algal family"))
ggplot()+
  geom_bar(data=melt1, aes(x=Tank, y=value, fill=variable),stat="identity")+
  geom_point(data=melt2, aes(x=Tank,y=value,color=variable),size=3)+
  geom_line(data=melt2, aes(x=Tank,y=value,group=variable,color=variable),size=1)+
  facet_wrap(~Type)+
  scale_y_continuous(name = expression("All Others"), sec.axis = sec_axis(~ ./10000000, name = "Maturity index"))+
  theme(axis.title.x=element_blank(),
        axis.title.y = element_text(color="grey46"),
        legend.title = element_blank(), 
        legend.position = "bottom",
        legend.direction = "vertical",
        legend.key.size = unit(0.3, "cm"))+
...
####################################################################################################################################
# plot 2
data <- read_excel("N:/DaheeKim/March/Result_analysis-graphs-2020(March).xlsx",sheet="plot2")
...
####################################################################################################################################
# plot 3
data <- read_excel("N:/DaheeKim/March/Result_analysis-graphs-2020(March).xlsx",sheet="plot3")
head(data)
melt <-reshape2::melt(data,id.vars="Tank")
...
####################################################################################################################################
# plot 4
data <- read_excel("N:/DaheeKim/March/Result_analysis-graphs-2020(March).xlsx",sheet="plot4")
head(data)
...
data$`Feed (G)/ shrimp` = data$`Feed (G)/ shrimp`*100
...
####################################################################################################################################
# plot 5
data <- read_excel("N:/DaheeKim/March/Result_analysis-graphs-2020(March).xlsx",sheet="plot5")
head(data)
melt <-reshape2::melt(data,id.vars="Tank")
table(melt$variable)
ls <- c("Total phytoplankton")
...
####################################################################################################################################
# plot 6
data <- read_excel("N:/DaheeKim/March/Result_analysis-graphs-2020(March).xlsx",sheet="plot6")
head(data)
...
####################################################################################################################################
# plot 7
data <- read_excel("N:/DaheeKim/March/Result_analysis-graphs-2020(March).xlsx",sheet="plot7")
head(data)
melt <-reshape2::melt(data,id.vars="Tank")
table(melt$variable)
...
####################################################################################################################################
# plot 8
data <- read_excel("N:/DaheeKim/March/Result_analysis-graphs-2020(March).xlsx",sheet="plot8")
head(data)
melt <-reshape2::melt(data,id.vars="Tank")
table(melt$variable)
library(scales)
...
data <- read_excel("C:/Users/daheekim/Desktop/Result_analysis-graphs-2020(results).xlsx",sheet="plot8")
head(data)
a <- ggplot() +
...
####################################################################################################################################
# plot 9
data <- read_excel("N:/DaheeKim/March/Result_analysis-graphs-2020(March).xlsx",sheet="plot9")
head(data)
melt <-reshape2::melt(data,id.vars="Tank")
table(melt$variable)
